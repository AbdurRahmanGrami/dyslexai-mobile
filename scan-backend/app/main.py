...
async def load_models():
    ...
    if not use_onnx:
        ...
        print("Loading T5 grammar correction (PyTorch)...")
        t5_tokenizer = T5Tokenizer.from_pretrained(T5_MODEL_NAME)
...
def groq_correct(text: str) -> str:
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    "You are an expert English Editor fixing OCR and dyslexic text. "
                    "Fix spelling and grammar from context. Fix proper nouns (e.g. Stakes->States). "
                    "Output ONLY the corrected text, no extra commentary."
                )},
